...
#pragma shader_feature UNITY_SINGLE_PASS_STEREO
	...
	#define CUSTOM_SAMPLE_SHADOW(tex,coord) tex2Dlod (tex,float4((coord).xyz,1)).r //is 1 for some reason
	...
	// Vita
	#define CUSTOM_DECLARE_SHADOWMAP(tex) sampler2D tex
	#define CUSTOM_SAMPLE_SHADOW(tex,coord) tex2Dlod<float>(tex, float4((coord).xyz,0))
	#define CUSTOM_SAMPLE_SHADOW_PROJ(tex,coord) tex2DprojShadow(tex, coord)
#elif defined(SHADER_API_PS3)
	#define CUSTOM_DECLARE_SHADOWMAP(tex) sampler2D tex
	#define CUSTOM_SAMPLE_SHADOW(tex,coord) tex2Dlod (tex,(float4(coord).xyz,0)).r
	#define CUSTOM_SAMPLE_SHADOW_PROJ(tex,coord) tex2Dproj (tex,coord).r
#else
	...
	#define CUSTOM_SAMPLE_SHADOW(tex,coord) ((SAMPLE_DEPTH_TEXTURE_LOD(tex,float4((coord).xy,0,0)) < (coord).z) ? 0.0 : 1.0)
	#define CUSTOM_SAMPLE_SHADOW_PROJ(tex,coord) ((SAMPLE_DEPTH_TEXTURE_PROJ(tex,UNITY_PROJ_COORD(coord)) < ((coord).z/(coord).w)) ? 0.0 : 1.0)
#endif
#else //old version of unity
...
//#endif
		...
		float MarchedDistance = length(start - _WorldSpaceCameraPos) * zScale;
		[loop]
